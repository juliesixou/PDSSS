...
```{r}
#Prénoms non-bretons pour le test de robustesse: on prend les 371 plus donnés du fichier des prénoms, sur toute la période, pour avoir autant de prénoms 
prenoms_alea <- read.csv("dpt2020.csv",sep=";",encoding = "UTF-8") %>%
  mutate(annais=as.numeric(annais)) %>%
  mutate(nombre=as.numeric(nombre)) %>%
  group_by(preusuel) %>% 
  summarise(nb_ts=sum(nombre)) %>% 
  top_n(n=371)

prenoms_alea <- prenoms_alea %>% filter(!(preusuel %in% listb$prenom) & preusuel != "_PRENOMS_RARES")

liste_alea <- c(prenoms_alea)$preusuel
prenoms <- prenoms %>% mutate(alea=ifelse(preusuel %in% liste_alea,1,0))
```
...
part68 <- prenoms %>%
  filter(annais==1968) %>%
  group_by(dpt) %>%
  summarise(freq=sum(nombre[breton==1]),freq_alea=sum(nombre[alea==1]))
...
part68 <- part68 %>%
  ...
  mutate(freq=freq/nb_naissances,freq_alea=freq_alea/nb_naissances) %>%
  ungroup () %>%
